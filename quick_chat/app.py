...
def is_user_online(user):
    """A user is online only if their last_seen is within the last 2 seconds."""
...
def api_conversations():
    ...
    for cid, convo in convos.items():
        ...
        result.append({
            ...
            "last_message": {
                "text": last_msg.get("text", "") if last_msg.get("type", "text") == "text" else ("📎 " + last_msg.get("text", "File") if last_msg.get("type") == "file" else ("🎤 Voice message" if last_msg.get("type") == "voice" else ("📍 Location" if last_msg.get("type") == "location" else ("📞 Call" if last_msg.get("type") == "call" else last_msg.get("text", ""))))),
...
def api_get_messages(conversation_id):
    ...
    for m in convo_msgs:
        ...
        # Server-side 15-minute window check (avoids timezone mismatch)
        try:
...
def api_get_stories():
    ...
    for story in stories:
        ...
        story_time = datetime.fromisoformat(story["created_at"])
...
def api_group_info(group_id):
    ...
    for mid in convo["members"]:
        ...
        member_details.append({
            ...
            "avatar_color": u.get("avatar_color", "#6c63ff"),
            "profile_pic": u.get("profile_pic", ""),
            "online": (datetime.now() - datetime.fromisoformat(u.get("last_seen", "2000-01-01"))).total_seconds() < 30 if u.get("last_seen") else False,
...
# Backfill username field on any pre‑existing stories that lack it.  This
# keeps old records from showing “Unknown” as the viewer cycles through;
# after running once the JSON is updated and the check in api_get_stories
