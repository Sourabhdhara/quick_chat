...
let pendingChatUser = null; // holds user object when a direct chat hasn't been created yet
let conversations = [];
let pollTimers = {};
let selectedUsers = [];
...
let lastMessageReadCounts = {}; // messageId -> read_count
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("contextmenu", (e) => {
        ...
        if (convoItem) {
            ...
            if (convoId) openConvoContextMenu(e, convoId);
        }
    });
    ...
    sendEmailOtpBtn.addEventListener("click", sendEmailVerification);
    confirmEmailBtn.addEventListener("click", confirmEmailVerification);
    ...
    conversationList.innerHTML = filtered.length === 0
        ...
        : filtered.map(c => {
            ...
            if (isGroup) {
                ...
                const picAttr = c.profile_pic ? `data-pic="${c.profile_pic}"` : "";
                avatarHtml = `<div class="avatar convo-avatar-clickable" style="background:${avatarBg};position:relative;overflow:hidden;cursor:pointer" ${picAttr} data-name="${escapeHtml(c.name)}" data-color="${avatarBg}" title="View profile photo">${avatarInner}</div>`;
            }
            ...
            return `
                <div class="convo-item ${activeClass}" data-id="${c.id}" onclick="openConversation('${c.id}')" oncontextmenu="openConvoContextMenu(event, '${c.id}')">
                    ${avatarHtml}
...
async function openConversation(id) {
    ...
    messageInput.focus();
    // Wire up contact info panel click for DM header
    setTimeout(() => attachDMContactClick(id), 0);
}
...
async function loadMessages(conversationId, force = false) {
    ...
    if (isEditingMessage) return;
        try {
            ...
            // Determine if we need to re-render (new message count OR read counts changed)
            let shouldRender = force || (newMsgCount !== lastMessageCount);

            if (!shouldRender && currentConversationId === conversationId && !isSelectMode) {
                // Check if any message's read_count changed
                for (const m of msgs) {
                    const prev = lastMessageReadCounts[m.id] || 0;
                    if ((m.read_count || 0) !== prev) {
                        shouldRender = true;
                        break;
                    }
                }
            ...
            // Update stored read counts for read receipt tracking
            lastMessageReadCounts = {};
            msgs.forEach(m => { lastMessageReadCounts[m.id] = m.read_count || 0; });
...
function renderMessages(msgs) {
    ...
    msgs.forEach(m => {
        ...
        const avatarHtml = !m.is_mine
            ? (m.profile_pic
                ? `<div class="avatar sm" style="overflow:hidden;background:transparent;cursor:pointer" onclick="handleAvatarClick(event,'${m.sender_id}','${escapeHtml(m.sender_name)}','${m.avatar_color||'#6c63ff'}')"><img src="${m.profile_pic}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"></div>`
                : `<div class="avatar sm" style="background:${m.avatar_color || '#6c63ff'};cursor:pointer" onclick="handleAvatarClick(event,'${m.sender_id}','${escapeHtml(m.sender_name)}','${m.avatar_color||'#6c63ff'}')">${m.sender_name[0].toUpperCase()}</div>`)
            : "";
...
function renderCallMessage(m) {
    ...
    if (callType === "video") {
        ...
        icon = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 5a2 2 0 012-2h3.5a2 2 0 011.8 1.1l1.2 2.6a2 2 0 01-.4 2.1l-1.5 1.5a16 16 0 006.6 6.6l1.5-1.5a2 2 0 012.1-.4l2.6 1.2A2 2 0 0121 17.5V21a2 2 0 01-2 2h-1C9.4 23 3 16.6 3 9V5z"/></svg>`;
    }
    ...
    return `<div class="call-event ${statusClass}">${icon} <span>${label}</span></div>`;
}
...
async function handleSendMessage(e) {
    ...
    // Determine where to send: existing conversation or pending direct chat
    let conversationId = currentConversationId;
    let recipientId = null;
    if (!conversationId && pendingChatUser) {
        recipientId = pendingChatUser.id;
    }
...
function renderDirectChatResults(users) {
    ...
    // Render and then attach click handlers (avoid inline JS string issues)
    userSearchResults.innerHTML = users.map(u => `
        <div class="user-result-item" data-user='${encodeURIComponent(JSON.stringify(u))}'>
            ${u.profile_pic
    ...
    document.querySelectorAll('.user-result-item').forEach(el => {
        const handler = () => {
            const userJson = el.getAttribute('data-user');
            if (!userJson) return;
            try {
...
async function startDirectChat(user) {
    // Navigate to an existing direct conversation if it exists.
    const existing = conversations.find(c => c.type === "direct" && c.members.includes(user.id) && c.members.includes(window.CURRENT_USER_ID));
    if (existing) {
        closeNewChatModal();
        await loadConversations();
    ...
    // Create a new direct conversation (hidden from the other user until a message is sent)
    try {
...
function formatTime(iso) {
    // parse with the same helper used elsewhere so naive strings are treated
    // consistently as local time (matching what the backend originally
    // generated) and UTC/Z‑suffixed values are correct.
    const d = _parseIsoTimestamp(iso);
...
function _parseIsoTimestamp(iso) {
    // Convert ISO string into a JS Date, treating *naive* values (no timezone
    // designator) as local time.  This matches how the server previously
    // generated strings (``datetime.now().isoformat()``) and prevents a
    // fixed offset from appearing when client and server are in different
    // timezones.
    if (!iso) return new Date(NaN);
    // Recognize strings like "2026-03-08T13:45:30" or with fractional
    // seconds.  They'll be parsed below without applying any offset.
    ...
    // Otherwise let the built‑in parser handle offsets/Z correctly.
    return new Date(iso);
}

function formatRelativeTime(iso) {
    if (!iso) return "";
    const date = _parseIsoTimestamp(iso);
...
async function openProfileModal() {
    try {
        ...
        if (data.email_verified) {
            profileEmailStatus.textContent = "Email verified";
            profileEmailStatus.style.color = "#16a34a";
        } else if (data.email) {
            profileEmailStatus.textContent = "Email not verified";
            profileEmailStatus.style.color = "#c2410c";
        } else {
            profileEmailStatus.textContent = "No email set";
            profileEmailStatus.style.color = "#6b7280";
        }
...
async function sendEmailVerification() {
    ...
    try {
        ...
        if (data.otp) {
            alert("Verification code (demo): " + data.otp);
        }

        profileEmailStatus.textContent = "Verification code sent";
        profileEmailStatus.style.color = "#0ea5e9";
...
// Keep convo ID for call record in case it ends before answering
    const callConvoId = currentConversationId;

    // Poll for answer
    pollForAnswer(callConvoId, callType);
...
function pollForAnswer(callConvoId, callType) {
    ...
    const interval = setInterval(async () => {
        ...
        try {
            ...
            if (data.status === "ended") {
                ...
                // If the call ended without an answer, record it as a missed call
                if (!data.answer) {
                    await postCallRecord(callConvoId, callType, "missed", 0, [window.CURRENT_USER_ID]);
                }
...
async function rejectCall() {
    ...
    // Record the rejected call in the chat history (hide it from the rejector)
    await postCallRecord(callConvoId, callType, "rejected", 0, [window.CURRENT_USER_ID]);
}
...
// ---------- Post call record to chat ----------
async function postCallRecord(conversationId, callType, callStatus, duration, deletedFor = []) {
    const labels = {
...
function renderGroupInfo() {
    ...
    membersList.innerHTML = d.members.map(m => {
        ...
        const avatarHtml = m.profile_pic
            ? `<div class="avatar" style="overflow:hidden;background:transparent;cursor:pointer" title="View photo" onclick="openPfpViewer('${m.profile_pic}','${escapeHtml(m.username)}','${m.avatar_color || '#6c63ff'}')"><img src="${m.profile_pic}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"></div>`
            : `<div class="avatar" style="background:${m.avatar_color};cursor:pointer" title="View photo" onclick="openPfpViewer(null,'${escapeHtml(m.username)}','${m.avatar_color || '#6c63ff'}')">${m.username[0].toUpperCase()}</div>`;
...
// ============================================================
//  PROFILE PICTURE VIEWER  (fullscreen lightbox)
//  Clicking ANY avatar — in messages, sidebar, group members,
//  chat header, story bar — opens a big fullscreen view.
// ============================================================

(function () {
    const overlay = document.getElementById("pfpViewerOverlay");
    const box     = document.getElementById("pfpViewerBox");
    const closeBtn = document.getElementById("pfpViewerClose");

    /** Open the viewer.
     * @param {string|null} picUrl   - URL of profile pic, or null
     * @param {string}      name     - Display name
     * @param {string}      color    - Fallback avatar bg colour
     */
    window.openPfpViewer = function (picUrl, name, color) {
        // Remove old content (keep close button)
        Array.from(box.children).forEach(el => {
            if (el !== closeBtn) el.remove();
        });

        if (picUrl) {
            const img = document.createElement("img");
            img.className = "pfp-viewer-img";
            img.src = picUrl;
            img.alt = name;
            box.appendChild(img);
        } else {
            const initials = document.createElement("div");
            initials.className = "pfp-viewer-initials";
            initials.style.background = color || "#6c63ff";
            initials.textContent = (name || "?")[0].toUpperCase();
            box.appendChild(initials);
        }

        const nameEl = document.createElement("div");
        nameEl.className = "pfp-viewer-name";
    ...
    closeBtn.addEventListener("click", closePfpViewer);
    overlay.addEventListener("click", (e) => {
        if (e.target === overlay) closePfpViewer();
    });
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape") closePfpViewer();
    });

    // -------------------------------------------------------
    //  Global delegated click handler — catches avatar clicks
    //  anywhere in the document.
    // -------------------------------------------------------
    document.addEventListener("click", function (e) {
        const target = e.target;

        // Was an <img> inside an .avatar clicked?
        const avatarImg   = target.closest(".avatar img");
        // Was an .avatar div itself clicked (initials)?
        const avatarDiv   = target.closest(".avatar");
        // Was a story avatar ring clicked?
        const storyAv     = target.closest(".story-av");

        if (!avatarImg && !avatarDiv && !storyAv) return;

        // Don't trigger if the click is inside profile edit modal, crop modal, etc.
        if (target.closest("#profileModal") ||
            target.closest("#cropModal")    ||
            target.closest("#newChatModal") ||
            target.closest(".attach-menu")  ||
            target.closest("#storyModal"))  return;

        // Don't trigger on upload overlays
        if (target.closest(".profile-pic-overlay") ||
            target.closest(".gi-pic-overlay")       ||
            target.closest(".gi-pic-actions"))      return;

        // Don't re-open if pfpViewer is already the event target
        if (target.closest(".pfp-viewer-overlay")) return;

        // Extract name + pic info from the nearest context
        let picUrl = null;
        let name   = "";
        let color  = "#6c63ff";

        const avatarEl = (avatarImg && avatarImg.closest(".avatar")) || avatarDiv || storyAv;

        if (avatarEl) {
            const img = avatarEl.querySelector("img");
            picUrl  = img ? img.src : null;
            color   = avatarEl.style.background || avatarEl.style.backgroundColor || "#6c63ff";
            // Try to read name from nearest siblings / data attributes
            const parent = avatarEl.parentElement;
            if (parent) {
                // conversation list item
                const nameEl = parent.querySelector(".convo-name");
                if (nameEl) { name = nameEl.textContent.trim(); }
                // message group
                const senderEl = parent.querySelector(".msg-sender");
                if (senderEl) { name = senderEl.textContent.trim(); }
                // group member item
                const memberNameEl = parent.querySelector(".gi-member-name");
                if (memberNameEl) { name = memberNameEl.textContent.trim(); }
                // story username
                const storyUserEl = parent.querySelector(".story-username");
                if (storyUserEl) { name = storyUserEl.textContent.trim(); }
            }
            // sidebar current user avatar
            if (avatarEl.id === "myAvatar" || avatarEl.id === "myStoryAvatar") {
                name  = window.CURRENT_USERNAME || name;
                // Don't hijack own avatar if it's part of the profile modal trigger
                if (avatarEl.closest("#currentUser")) return;
            }
            // chat header avatar
            if (avatarEl.id === "chatAvatar") {
                name = document.getElementById("chatName").textContent.trim();
            }
        }

        if (!name && !picUrl) return; // nothing useful — ignore

        e.stopPropagation();
        openPfpViewer(picUrl, name, color);
    }, true); // capture phase so we beat other handlers
})();


// ============================================================
//  CONTACT INFO PANEL  (WhatsApp-style, for DMs only)
//  Opens when the user clicks the name / avatar in the chat
//  header of a direct-message conversation.
// ============================================================

(function () {
    const panel    = document.getElementById("contactInfoPanel");
    const body     = document.getElementById("contactInfoBody");
    const closeBtn = document.getElementById("closeContactInfoBtn");

    let _currentUserId = null;

    function fmt_last_seen(iso) {
    ...
    window.openContactInfo = async function (userId) {
        if (!userId) return;
        ...
        body.innerHTML = `
            <div class="ci-hero">
                <div class="ci-avatar-initials" style="background:#d1d5db"></div>
                <div class="ci-name" style="background:#e5e7eb;border-radius:8px;width:120px;height:18px;"></div>
            </div>`;
        panel.classList.add("open");
        ...
        try {
            ...
            if (!res.ok) throw new Error(user.error || "Failed");

            const hasPic    = !!user.profile_pic;
            const initial   = (user.username || "?")[0].toUpperCase();
            const color     = user.avatar_color || "#6c63ff";
            const isOnline  = user.online;
            const statusTxt = isOnline ? "Online" : `Last seen ${fmt_last_seen(user.last_seen)}`;
            const bio       = user.bio ? escapeHtml(user.bio) : null;
            const joined    = fmt_joined(user.created_at);

            const avatarHtml = hasPic
                ? `<img class="ci-avatar-img" src="${user.profile_pic}" alt="${escapeHtml(user.username)}">`
                : `<div class="ci-avatar-initials" style="background:${color}">${initial}</div>`;

            body.innerHTML = `
                <div class="ci-hero">
                    <div class="ci-avatar-wrap" id="ciAvatarWrap" title="View photo">
                        ${avatarHtml}
                        <div class="ci-zoom-hint">
                            <svg width="28" height="28" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round">
                                <circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16" y2="16"/>
                                <line x1="8" y1="11" x2="14" y2="11"/><line x1="11" y1="8" x2="11" y2="14"/>
                            </svg>
                        </div>
                    </div>
                    <div class="ci-name">${escapeHtml(user.username)}</div>
                    <div class="ci-status-badge">
                        <span class="ci-status-dot ${isOnline ? 'online' : ''}"></span>
                        ${escapeHtml(statusTxt)}
                    </div>
                </div>

                <div class="ci-section-gap"></div>
                ...
                <div class="ci-section">
                    <div class="ci-row">
                        ...
                        <div class="ci-row-content">
                            <div class="ci-row-label">About</div>
                            <div class="ci-row-value ${bio ? '' : 'muted'}">${bio || 'No about info'}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="ci-section">
                    ...
                    <div class="ci-row">
                        ...
                        <div class="ci-row-content">
                            <div class="ci-row-label">Member since</div>
                            <div class="ci-row-value">${escapeHtml(joined)}</div>
                        </div>
                    </div>` : ''}
                </div>`;

            // Clicking the avatar inside panel → open pfp viewer
            const wrap = document.getElementById("ciAvatarWrap");
            if (wrap) {
                wrap.addEventListener("click", (e) => {
                    e.stopPropagation();
                    openPfpViewer(hasPic ? user.profile_pic : null, user.username, color);
                });
            }

        } catch (err) {
            console.error("Failed to load contact info:", err);
            body.innerHTML = `<div style="padding:32px;text-align:center;color:#6b7280;">Failed to load contact info.</div>`;
        }
    };
    ...
    // Close when clicking outside panel (on chat area / backdrop)
    document.addEventListener("click", (e) => {
        if (panel.classList.contains("open") &&
            !panel.contains(e.target) &&
            !e.target.closest(".chat-contact")) {
            closeContactInfo();
        }
    });

    // attachDMContactClick — called from openConversation after each navigation
    window.attachDMContactClick = function(id) {
        const convo = (window.conversations || []).find(c => c.id === id);
        const chatContact = document.querySelector(".chat-contact");
        if (!chatContact || !convo) return;

        if (convo.type === "direct") {
            const otherId = (convo.members || []).find(m => m !== window.CURRENT_USER_ID);
            if (otherId) {
                chatContact.style.cursor = "pointer";
                chatContact.onclick = (e) => {
                    e.stopPropagation();
                    openContactInfo(otherId);
                };
            }
        }
        // Groups keep their existing openGroupInfo() handler set in the original openConversation
    };
})();

// ============================================================
//  handleAvatarClick — called from message bubble avatars
//  In a group chat: opens pfp viewer (anyone can see pic)
//  In a direct chat: opens contact info panel
// ============================================================
function handleAvatarClick(e, senderId, senderName, senderColor) {
    e.stopPropagation();
    // Determine if current convo is a group or direct
    const convo = (window.conversations || []).find(c => c.id === window.currentConversationId);
    if (convo && convo.type === "group") {
        // In groups: open pfp viewer directly
        // Find profile_pic from the message avatar's img src
        const avatarEl = e.currentTarget || e.target.closest(".avatar");
        const img = avatarEl ? avatarEl.querySelector("img") : null;
        openPfpViewer(img ? img.src : null, senderName, senderColor);
    } else {
        // In DMs: open full contact info panel
        openContactInfo(senderId);
    }
}

// Delegated click for conversation list avatars (direct chat pfp viewer)
document.addEventListener("click", function(e) {
    const av = e.target.closest(".convo-avatar-clickable");
    if (!av) return;
    e.stopPropagation();
    const pic   = av.dataset.pic   || null;
    const name  = av.dataset.name  || "";
    const color = av.dataset.color || "#6c63ff";
    openPfpViewer(pic, name, color);
});
